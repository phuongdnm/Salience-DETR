...
####### START OF THE NEW BACKBONE ################
mobilenet = timm.create_model(
    "hf_hub:timm/mobilenetv4_conv_aa_large.e230_r448_in12k_ft_in1k",
    pretrained=True,
    features_only=True,
    out_indices=(2, 3, 4),
)
...
# freeze the backbone
for module in [mobilenet.blocks[0], mobilenet.blocks[1]]:
    module.eval()
...
class TIMMBackboneWrapper(nn.Module):
    def __init__(self, model):
        super().__init__()
        self.model = model
        self.num_channels = model.feature_info.channels()

    def forward(self, image: torch.Tensor):
        features = self.model(image)

        # convert from list to dict, keys can be anything, we simply use the index
        features = dict(enumerate(features))

        return features


backbone = TIMMBackboneWrapper(model=mobilenet)
